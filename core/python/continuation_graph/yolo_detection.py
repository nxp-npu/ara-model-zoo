...
def continue_yolo_detection(
    ...
    *,
    ...
    scores = as_channel_first(
        find_output(model_output.outputs, "Sigmoid_output"), num_classes
    )
    predictions = np.concatenate((boxes, scores), axis=1).astype(np.float32, copy=False)
