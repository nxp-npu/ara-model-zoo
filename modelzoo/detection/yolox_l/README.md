...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolox_l
```


- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/detection/yolox_l/output/assets` folder

#### Model Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolox_l dataset_root=<path to val2017>
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../images model=yolox_l
./flows/infer_hw.sh images_folder=../images model=yolox_l
```

- Output images saved in `modelzoo/detection/yolox_l/output/postprocessed_output_visualized`
...
```bash
cd ara-model-zoo
./flows/eval_float.sh dataset_root='../coco2017/' model=yolox_l
./flows/eval_hw.sh dataset_root='../coco2017/' model=yolox_l
```
