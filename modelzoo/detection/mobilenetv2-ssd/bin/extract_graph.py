...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", required=True, help="Path to the model run.yaml")
...
def load_cutoff_outputs(config_path: Path) -> list[str]:
    ...
    return [
        name.replace("#", ":") for name in re.split(r"[:\s]+", onode.strip()) if name
    ]
...
def main() -> None:
    ...
    cutoff_outputs = load_cutoff_outputs(Path(args.config))
    ...
    print(f"Generated postprocessing.onnx with outputs {output_names}")
