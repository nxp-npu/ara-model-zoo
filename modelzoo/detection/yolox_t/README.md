...
The model input is a blob that consists of a single image of `1, 3, 416, 416`.
...
Accuracy metrics reflect IOU threshold of 0.65 and NMS threshold of 0.001, during evaluation.
...
| Parameter            | Value                                                 |
| :------------------- | :---------------------------------------------------- |
| `Model input shape`  | `[3,416,416]`                                         |
| `Model output shape` | `[1, 3549, 85]` |
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolox_t
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/detection/yolox_t/output/assets` folder

#### Model Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolox_t dataset_root=<path to val2017>
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../images model=yolox_t
./flows/infer_hw.sh images_folder=../images model=yolox_t
```

- Output images saved in `modelzoo/detection/yolox_t/output/postprocessed_output_visualized`
...
```bash
cd ara-model-zoo
./flows/eval_float.sh dataset_root='../coco2017/' model=yolox_t
./flows/eval_hw.sh dataset_root='../coco2017/' model=yolox_t
```
