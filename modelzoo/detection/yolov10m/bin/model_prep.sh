...
info "Exporting yolov10m to ONNX (batch_size=${BATCH_SIZE})..."
python3 <<'HEREDOC'
import os
...
if [[ ! -f "${YOLOV10_REPO}/yolov10m.onnx" ]]; then
  error "ONNX export failed: ${YOLOV10_REPO}/yolov10m.onnx was not created"
  return 1
fi

mv "${YOLOV10_REPO}/yolov10m.onnx" "${ONNX_OUTPUT}"
