...
info "Exporting yolov10s to ONNX (batch_size=${BATCH_SIZE})..."
python3 <<'HEREDOC'
import os
...
if [[ ! -f "${YOLOV10_REPO}/yolov10s.onnx" ]]; then
  error "ONNX export failed: ${YOLOV10_REPO}/yolov10s.onnx was not created"
  return 1
fi

mv "${YOLOV10_REPO}/yolov10s.onnx" "${ONNX_OUTPUT}"
