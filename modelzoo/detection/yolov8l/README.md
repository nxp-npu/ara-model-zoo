...
YOLOv8l is an object detection model developed by ultralytics and a part of YOLO(You Only Look Once) family.  The model is pretrained with Coco2017 dataset

YOLOv8l is a large model in the YOLOv8 family. It's designed for environments where computational resources are limited, such as on mobile devices or edge devices. While it may not offer the same level of accuracy as the larger models, it's faster and requires less computational resources, making it a good choice for real-time object detection tasks in resource-constrained environments.
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolov8l
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/face_detection/yolov8l/output/compiled_model` folder
...
```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolov8l dataset_root=<path to val2017>
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../images model=yolov8l
./flows/infer_hw.sh images_folder=../images model=yolov8l
```

- Output images saved in `modelzoo/face_detection/yolov8l/output/postprocessed_output_visualized`
...
```bash
cd ara-model-zoo
./flows/run_eval_float.sh dataset_root=<folder containing image dataset> model=yolov8l
./flows/run_eval_hw.sh dataset_root=<folder containing image dataset> model=yolov8l
```
