...
Model is being saved from YOLOX github repository.
repository can be found [here](https://github.com/Megvii-BaseDetection/YOLOX.git)
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolox_m
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/detection/yolox_m/output/assets` folder
...
```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolox_m dataset_root=<path to val2017>
```

- Output model and assets will be stored in the `modelzoo/detection/yolox_m/output/compiled_model` folder
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../images model=yolox_m
./flows/infer_hw.sh images_folder=../images model=yolox_m
```

- Output images saved in `modelzoo/detection/yolox_m/output/postprocessed_output_visualized`
...
```bash
cd ara-model-zoo
./flows/eval_float.sh dataset_root='../coco2017/' model=yolox_m
./flows/eval_hw.sh dataset_root='../coco2017/' model=yolox_m
```
