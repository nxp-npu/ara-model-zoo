...
YOLOv7x is an object detection model and is a part of YOLO(You Only Look Once) family.
...
[Yolov7x-onnx](https://github.com/WongKinYiu/yolov7)
...
The FP32 Value is taken from the github source of the present model [Yolov7x-onnx](https://github.com/WongKinYiu/yolov7)
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolov7x
```
- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/detection/yolov7x/output/assets` folder

#### Model Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolov7x dataset_root=<path to val2017>
```
- Output model and assets will be stored in the `modelzoo/detection/yolov7x/output/assets` folder
...
```bash
cd ara-model-zoo
./flows/performance.sh model=yolov7x
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh model=yolov7x images_folder=<path to images to run inference on>
./flows/infer_hw.sh model=yolov7x images_folder=<path to images to run inference on>
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../testimages model=yolov7x
./flows/infer_hw.sh images_folder=../testimages model=yolov7x
```

- Output images saved in `modelzoo/detection/yolov7x/output/postprocessed_output_visualized`
...
```bash
cd ara-model-zoo
./flows/eval_float.sh model=yolov7x dataset_root=<folder containing image dataset>
./flows/eval_hw.sh model=yolov7x dataset_root=<folder containing image dataset>
```
...
```bash
cd ara-model-zoo
./flows/eval_hw.sh model=yolov7x dataset_root=/media/data/datasets/coco2017
```
