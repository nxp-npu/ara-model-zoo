# MobilenetV1 SSD (TensorFlow)

## Use Case and High-Level Description

Mobilenet V1 SSD is an object detection model pre-trained on COCO 2017 dataset. This is the TensorFlow implementation.

The model input is a blob that consists of a single image of `1, 3, 300, 300`.

## Model Source

The model is taken from [this table](https://github.com/tensorflow/models/blob/master/research/object_detection/g3doc/tf1_detection_zoo.md) ssd-checkpoints for `ssd_mobilenet_v1_coco`
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=mobilenetv1-ssd
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/detection/mobilenetv1-ssd/output/assets` folder
...
```bash
cd ara-model-zoo
./flows/model_compile.sh model=mobilenetv1-ssd dataset_root=<path to val2017>
```

- Output model and assets will be stored in the `modelzoo/detection/mobilenetv1-ssd/output/compiled_model` folder

#### Performance checker

```bash
cd ara-model-zoo
./flows/performance.sh model=mobilenetv1-ssd
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../images model=mobilenetv1-ssd
./flows/infer_hw.sh images_folder=../images model=mobilenetv1-ssd
```

- Output images saved in `modelzoo/detection/mobilenetv1-ssd/output/postprocessed_output_visualized`

#### Accuracy checker

```bash
cd ara-model-zoo
./flows/eval_hw.sh dataset_root='../coco2017/' model=mobilenetv1-ssd
./flows/eval_float.sh dataset_root='../coco2017/' model=mobilenetv1-ssd
```
