...
YOLOv11n is an object detection model developed by ultralytics and a part of YOLO(You Only Look Once) family. The model is pretrained with Coco2017 dataset

YOLOv11n is the smallest model in the YOLOv11 family. It's designed for environments where computational resources are limited, such as on mobile devices or edge devices. While it may not offer the same level of accuracy as the larger models, it's faster and requires less computational resources, making it a good choice for real-time object detection tasks in resource-constrained environments.
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolov11n
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/detection/yolov11n/output/assets` folder
...
```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolov11n dataset_root=<path to val2017>
```

- Output model and assets will be stored in the `modelzoo/detection/yolov11n/output/compiled_model` folder
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../images model=yolov11n
./flows/infer_hw.sh images_folder=../images model=yolov11n
```

- Output images saved in `modelzoo/detection/yolov11n/output/postprocessed_output_visualized`
...
```bash
cd ara-model-zoo
./flows/eval_float.sh dataset_root='../coco2017/' model=yolov11n
./flows/eval_hw.sh dataset_root='../coco2017/' model=yolov11n
```
