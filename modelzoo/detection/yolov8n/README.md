...
YOLOv8n is an object detection model developed by ultralytics and a part of YOLO(You Only Look Once) family.  The model is pretrained with Coco2017 dataset
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolov8n
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/face_detection/yolov8n/output/compiled_model` folder
...
```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolov8n dataset_root=<path to val2017>
```
...
```bash
./flows/performance.sh model=yolov8n
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=<path_to_images_folder> model=yolov8n
./flows/infer_hw.sh images_folder=<path_to_images_folder> model=yolov8n
```

- Output images saved in `modelzoo/face_detection/yolov8n/output/postprocessed_output_visualized`
...
```bash
cd ara-model-zoo
./flows/run_eval_float.sh dataset_root=<folder containing image dataset> model=yolov8n
./flows/run_eval_hw.sh dataset_root=<folder containing image dataset> model=yolov8n
```
