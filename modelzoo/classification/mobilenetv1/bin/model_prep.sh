...
_model_prep_cleanup() {
  ...
  rm -rf "$OUTPUT_DIR"/mobilenet_v1_1.0_224.tgz  "$MOBILENET_REPO"
}
...
unset PYTHONPATH
virtualenv mobilenetv1app -p python3
source mobilenetv1app/bin/activate
...
wget http://download.tensorflow.org/models/mobilenet_v1_2018_08_02/mobilenet_v1_1.0_224.tgz -P "$OUTPUT_DIR"
tar -xvf "$OUTPUT_DIR"/mobilenet_v1_1.0_224.tgz  -C "$MOBILENET_REPO"
...
python3 -m tf2onnx.convert \
  --graphdef "$MOBILENET_REPO"/mobilenet_v1_1.0_224_frozen.pb \
  --output "$ONNX_OUTPUT" \
...
OUTPUT_OLD = "MobilenetV1/Predictions/Softmax:0"
OUTPUT_NEW = "MobilenetV1/Predictions/Softmax"
