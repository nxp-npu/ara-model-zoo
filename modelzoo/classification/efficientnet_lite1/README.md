...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=efficientnet_lite1
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/classification/efficientnet_lite1/output/compiled_model` folder

### Model Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=efficientnet_lite1 dataset_root=<path to imagenet directory>
```

Output model is stored in `modelzoo/classification/efficientnet_lite1/output/compiled_model/`

#### Performance Checker

```bash
cd ara-model-zoo
./flows/performance.sh model=efficientnet_lite1
```
...
```bash
cd ara-model-zoo
./flows/infer_hw.sh model=efficientnet_lite1 images_folder=./testimages
./flows/infer_float.sh model=efficientnet_lite1 images_folder=./testimages
```
...
```bash
cd ara-model-zoo
./flows/eval_hw.sh model=efficientnet_lite1 dataset_root=/media/data/datasets/imagenet
./flows/eval_float.sh model=efficientnet_lite1 dataset_root=<path to imagenet directory>
```
