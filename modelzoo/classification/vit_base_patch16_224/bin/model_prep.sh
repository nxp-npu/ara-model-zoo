...
VIT_BASE_PATCH16_224="vit_base_patch16_224_virtual_env"
MODEL_FILE_NAME="vit_base_patch16_224"
...
_model_prep_cleanup() {
  info "Cleaning up temporary vit_base_patch16_224 repository..."
  deactivate 2>/dev/null || true
  rm -rf "${VIT_BASE_PATCH16_224}"
...
virtualenv -p python3 "${VIT_BASE_PATCH16_224}"
source "${VIT_BASE_PATCH16_224}/bin/activate"
...
# ─── Export to ONNX ──────────────────────────────────────────────────────────
# vit uses scaled_dot_product_attention which requires ONNX opset >= 14.
