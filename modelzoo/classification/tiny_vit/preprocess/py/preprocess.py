...
# This preprocess function is necessary to handle the special case of Tiny ViT model where BGR to RGB and Resize operations are handled outside of the main preprocessing pipeline. 
# This is because in the original implementation of Tiny ViT, these two operations are performed together before all other preprocessing steps and they are not part of the main preprocessing pipeline. 
# Hence we need to handle them separately in this preprocess function before executing the remaining preprocessing pipeline.
