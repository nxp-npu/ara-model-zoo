...
ResNet 18d is image classification model pre-trained on ImageNet dataset. This
is PyTorch\* implementation available in the Hugging Face model repository. You can find the original model source [here](https://huggingface.co/timm/resnet18d.ra2_in1k)
...
This model is being taken directly from [Hugging Face](https://huggingface.co/timm/resnet18d.ra2_in1k) library.

## Specification

More Information specific to this Particular model can be found here [resnet18d](https://huggingface.co/timm/resnet18d.ra2_in1k)
...
The FP32 Value is taken from Pytorch Documentation for [resnet18d](https://huggingface.co/timm/resnet18d.ra2_in1k)
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=resnet18d
```
* Precompiled models are compiled using Ara SDK.
* Output model and assets will be stored in the `modelzoo/classification/resnet18d/output/assets` folder

#### Model Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=resnet18d dataset_root=<path to imagenet directory>
```
* Output model and assets will be stored in the `modelzoo/classification/resnet18d/output/assets` folder
...
```bash
cd ara-model-zoo
./flows/performance.sh model=resnet18d
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh model=resnet18d images_folder=<path to images to run inference on>
```
...
```bash
cd ara-model-zoo
./flows/infer_hw.sh images_folder=../testimages model=resnet18d
```
...
```bash
cd ara-model-zoo
./flows/eval_hw.sh model=resnet18d dataset_folder=<folder containing image dataset>
```
...
```bash
cd ara-model-zoo
./flows/eval_hw.sh model=resnet18d dataset_root=/media/data/datasets/imagenet/imagenet
```
