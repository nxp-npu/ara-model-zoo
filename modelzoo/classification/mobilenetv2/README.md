...
MobileNet v2 is an image classification model which has been pre-trained on the ImageNet dataset.
...
The model is taken from [this table](https://github.com/tensorflow/models/blob/master/research/slim/nets/mobilenet/README.md#mobilenet-v2-imagenet-checkpoints) for `float_v2_1.0_224`
...
```bash
./flows/precompiled_model_download.sh model=mobilenetv2
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/classification/mobilenetv2/output/assets` and `modelzoo/classification/mobilenetv2/output/compiled_model` folder

#### Model Compile

```bash
./flows/model_compile.sh model=mobilenetv2 dataset_root=<folder containing calibration image dataset>
```
* Output model and assets will be stored in the `modelzoo/classification/mobilenetv2/output/assets` folder


#### Performance checker

```bash
./flows/performance.sh model=mobilenetv2
```
...
```bash
./flows/eval_float.sh model=mobilenetv2 dataset_root=<folder containing image dataset>
```

##### HW Evaluation

```bash
./flows/eval_hw.sh model=mobilenetv2 dataset_root=<folder containing image dataset>
```
