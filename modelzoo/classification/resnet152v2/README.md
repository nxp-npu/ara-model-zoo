...
More Information specific to this Particular model can be found here [resnet152 IMAGENET1K_V2](https://docs.pytorch.org/vision/main/models/generated/torchvision.models.resnet152.html)
...
The FP32 Value is taken from Pytorch Documentation for [resnet152 IMAGENET1K_V2](https://docs.pytorch.org/vision/main/models/generated/torchvision.models.resnet152.html)
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=resnet152v2
```
* Precompiled models are compiled using Ara SDK.
* Output model and assets will be stored in the `modelzoo/classification/resnet152v2/output/assets` folder

#### Model Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=resnet152v2 dataset_root=<path to imagenet directory>
```
* Output model and assets will be stored in the `modelzoo/classification/resnet152v2/output/assets` folder
...
```bash
cd ara-model-zoo
./flows/performance.sh model=resnet152v2
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh model=resnet152v2 images_folder=<path to images to run inference on>
```
...
```bash
cd ara-model-zoo
./flows/infer_hw.sh images_folder=../testimages model=resnet152v2
```
...
```bash
cd ara-model-zoo
./flows/eval_hw.sh model=resnet152v2 dataset_folder=<folder containing image dataset>
```
...
```bash
cd ara-model-zoo
./flows/eval_hw.sh model=resnet152v2 dataset_root=/media/data/datasets/imagenet/imagenet
```
