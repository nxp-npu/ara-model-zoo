...
More Information specific to this Particular model can be found here [resnet34 IMAGENET1K_V1](https://pytorch.org/vision/main/models/generated/torchvision.models.resnet34.html)
...
The FP32 Value is taken from Pytorch Documentation for [resnet34 IMAGENET1K_V1](https://pytorch.org/vision/main/models/generated/torchvision.models.resnet34.html)
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=resnet34
```
* Precompiled models are compiled using Ara SDK.
* Output model and assets will be stored in the `modelzoo/classification/resnet34/output/assets` folder

#### Model Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=resnet34 dataset_root=<path to imagenet directory>
```
* Output model and assets will be stored in the `modelzoo/classification/resnet34/output/assets` folder
...
```bash
cd ara-model-zoo
./flows/performance.sh model=resnet34
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh model=resnet34 images_folder=<path to images to run inference on>
```
...
```bash
cd ara-model-zoo
./flows/infer_hw.sh images_folder=../testimages model=resnet34
```
...
```bash
cd ara-model-zoo
./flows/eval_hw.sh model=resnet34 dataset_folder=<folder containing image dataset>
```
...
```bash
cd ara-model-zoo
./flows/eval_hw.sh model=resnet34 dataset_root=/media/data/datasets/imagenet/imagenet
```
