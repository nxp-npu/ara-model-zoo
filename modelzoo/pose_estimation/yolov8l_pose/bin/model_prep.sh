...
info "Exporting yolov8l_pose to ONNX with fixed batch size ${EXPORT_BATCH_SIZE}..."
...
onnx_path = Path("yolov8l-pose.onnx")
...
if [[ ! -f "${YOLOV8_REPO}/yolov8l-pose.onnx" ]]; then
  error "ONNX export failed: ${YOLOV8_REPO}/yolov8l-pose.onnx was not created"
  return 1
fi

mv "${YOLOV8_REPO}/yolov8l-pose.onnx" "${ONNX_OUTPUT}"
if [[ -f "${YOLOV8_REPO}/yolov8l-pose.pt" ]]; then
  mv "${YOLOV8_REPO}/yolov8l-pose.pt" "${PT_OUTPUT}"
fi
