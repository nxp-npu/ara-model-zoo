...
info "Exporting yolov8x_pose to ONNX with fixed batch size ${EXPORT_BATCH_SIZE}..."
MODEL_PREP_BATCH_SIZE="${EXPORT_BATCH_SIZE}" python3 <<'HEREDOC'
import os
...
onnx_path = Path("yolov8x-pose.onnx")
...
if [[ ! -f "${YOLOV8_REPO}/yolov8x-pose.onnx" ]]; then
  error "ONNX export failed: ${YOLOV8_REPO}/yolov8x-pose.onnx was not created"
  return 1
fi

mv "${YOLOV8_REPO}/yolov8x-pose.onnx" "${ONNX_OUTPUT}"
if [[ -f "${YOLOV8_REPO}/yolov8x-pose.pt" ]]; then
  mv "${YOLOV8_REPO}/yolov8x-pose.pt" "${PT_OUTPUT}"
fi
