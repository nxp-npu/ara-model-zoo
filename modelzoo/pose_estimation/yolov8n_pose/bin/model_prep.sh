...
onnx_path = Path("yolov8n-pose.onnx")
...
if [[ ! -f "${YOLOV8_REPO}/yolov8n-pose.onnx" ]]; then
  error "ONNX export failed: ${YOLOV8_REPO}/yolov8n-pose.onnx was not created"
  return 1
fi

mv "${YOLOV8_REPO}/yolov8n-pose.onnx" "${ONNX_OUTPUT}"
if [[ -f "${YOLOV8_REPO}/yolov8n-pose.pt" ]]; then
  mv "${YOLOV8_REPO}/yolov8n-pose.pt" "${PT_OUTPUT}"
fi
