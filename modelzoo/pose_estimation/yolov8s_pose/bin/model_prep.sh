...
info "Exporting yolov8s_pose to ONNX with fixed batch size ${EXPORT_BATCH_SIZE}..."
...
onnx_path = Path("yolov8s-pose.onnx")
...
if [[ ! -f "${YOLOV8_REPO}/yolov8s-pose.onnx" ]]; then
  error "ONNX export failed: ${YOLOV8_REPO}/yolov8s-pose.onnx was not created"
  return 1
fi

mv "${YOLOV8_REPO}/yolov8s-pose.onnx" "${ONNX_OUTPUT}"
if [[ -f "${YOLOV8_REPO}/yolov8s-pose.pt" ]]; then
  mv "${YOLOV8_REPO}/yolov8s-pose.pt" "${PT_OUTPUT}"
fi
