...
YOLOv8s-pose is a human pose estimation model from the YOLOv8 family developed by Ultralytics. The model is pretrained on the COCO 2017 keypoints dataset and predicts person bounding boxes with 17 body keypoints.

YOLOv8s-pose is the small variant. It provides a higher accuracy target than the nano model while keeping compute and memory requirements low for edge inference.
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolov8s_pose
```

- Pre-compiled models are compiled using the Ara SDK.
- Output model and assets will be stored in `modelzoo/pose_estimation/yolov8s_pose/output/assets`.
- A copy of `model.dvm` will be stored in `modelzoo/pose_estimation/yolov8s_pose/output/compiled_model`.

### Manual Model Preparation and Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolov8s_pose dataset_root=<path_to_coco_keypoints_data> run=python
```

- Output model and assets will be stored in `modelzoo/pose_estimation/yolov8s_pose/output/compiled_model`.
...
```bash
cd ara-model-zoo
./flows/performance.sh model=yolov8s_pose
```
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=<path_to_images_folder> model=yolov8s_pose
./flows/infer_hw.sh images_folder=<path_to_images_folder> model=yolov8s_pose
```

- Output images are saved under `modelzoo/pose_estimation/yolov8s_pose/output/postprocessed_output_visualized`.
...
```bash
cd ara-model-zoo
./flows/eval_hw.sh model=yolov8s_pose dataset_root=<path_to_coco_keypoints_data>
```

- `dataset_root` should contain `val2017` images and `person_keypoints_val2017.json`.
- Accuracy metrics are printed to the terminal and written under `modelzoo/pose_estimation/yolov8s_pose/output/evaluation`.
