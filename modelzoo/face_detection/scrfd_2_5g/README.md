...
SCRFD-2.5G is an efficient SCRFD face detector with five facial landmarks. It offers a larger compute budget than SCRFD-500M while keeping the same face detection pipeline and output structure.
...
| --- | --- | --- |
| Easy Val AP | 93.93% | 93.85% |
| Medium Val AP | 92.06% | 91.89% |
| Hard Val AP | 76.96% | 76.65% |
...
```bash
./flows/precompiled_model_download.sh model=scrfd_2_5g
```

* The DVM is stored as `modelzoo/face_detection/scrfd_2_5g/output/compiled_model/model.dvm`.
* This flow does not download the ONNX model; use the model compilation flow below to prepare it.

#### Model Compile

```bash
./flows/model_compile.sh model=scrfd_2_5g dataset_root=<path to WIDER FACE>
```

* The ONNX and DVM files are stored in `modelzoo/face_detection/scrfd_2_5g/output/compiled_model`; compilation assets are generated in `modelzoo/face_detection/scrfd_2_5g/output/assets`.

#### Performance checker

```bash
./flows/performance.sh model=scrfd_2_5g
```

The flow requires a compiled DVM and uses `sudo` to start the hardware proxy unless `skip_proxy=true`. Results are written to `modelzoo/face_detection/scrfd_2_5g/output/performance`.
...
```bash
./flows/infer_float.sh model=scrfd_2_5g images_folder=<path to images>
```

Hardware inference:

```bash
./flows/infer_hw.sh model=scrfd_2_5g images_folder=<path to images>
```

* Output images are saved in `modelzoo/face_detection/scrfd_2_5g/output/postprocessed_output_visualized`.
...
```bash
./flows/eval_float.sh model=scrfd_2_5g dataset_root=<path to WIDER FACE>
```

Hardware evaluation:

```bash
./flows/eval_hw.sh model=scrfd_2_5g dataset_root=<path to WIDER FACE>
```
