...
MODEL_URL="https://github.com/yakhyo/facial-analysis/releases/download/v0.0.1/det_10g.onnx"
CLASSIFICATION_NODES=("441" "464" "487")
BOX_NODES=("443" "466" "489")
KEYPOINT_NODES=("444" "467" "490")

ASSETS_DIR="${OUTPUT_DIR}/compiled_model"
WORK_DIR="${OUTPUT_DIR}/model_prep"
VENV_DIR="${OUTPUT_DIR}/scrfdapp"
RAW_ONNX="${WORK_DIR}/scrfd_10g.raw.onnx"
SIMPLIFIED_ONNX="${WORK_DIR}/scrfd_10g.sim.onnx"
...
info "Creating virtual environment for SCRFD-10G export..."
unset PYTHONPATH
...
info "Downloading SCRFD-10G ONNX model..."
wget --no-verbose -O "${RAW_ONNX}" "${MODEL_URL}"

info "Simplifying SCRFD-10G ONNX with fixed batch size ${EXPORT_BATCH_SIZE}..."
python -m onnxsim \
  "${RAW_ONNX}" \
  "${SIMPLIFIED_ONNX}" \
  --overwrite-input-shape "${EXPORT_BATCH_SIZE},3,640,640"

info "Pruning SCRFD-10G ONNX outputs..."
...
info "Saved SCRFD-10G ONNX model to ${ONNX_OUTPUT}"
