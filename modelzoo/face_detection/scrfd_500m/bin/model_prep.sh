...
MODEL_URL="https://github.com/yakhyo/face-reidentification/releases/download/v0.0.1/det_500m.onnx"
CLASSIFICATION_NODES=("437" "462" "487")
BOX_NODES=("438" "463" "488")
KEYPOINT_NODES=("439" "464" "489")

ASSETS_DIR="${OUTPUT_DIR}/compiled_model"
WORK_DIR="${OUTPUT_DIR}/model_prep"
VENV_DIR="${OUTPUT_DIR}/scrfdapp"
RAW_ONNX="${WORK_DIR}/scrfd_500m.raw.onnx"
SIMPLIFIED_ONNX="${WORK_DIR}/scrfd_500m.sim.onnx"
...
info "Creating virtual environment for SCRFD-500M export..."
unset PYTHONPATH
...
info "Downloading SCRFD-500M ONNX model..."
wget --no-verbose -O "${RAW_ONNX}" "${MODEL_URL}"

info "Simplifying SCRFD-500M ONNX with fixed batch size ${EXPORT_BATCH_SIZE}..."
python -m onnxsim \
  "${RAW_ONNX}" \
  "${SIMPLIFIED_ONNX}" \
  --overwrite-input-shape "${EXPORT_BATCH_SIZE},3,640,640"

info "Pruning SCRFD-500M ONNX outputs..."
...
info "Saved SCRFD-500M ONNX model to ${ONNX_OUTPUT}"
