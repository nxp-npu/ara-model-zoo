...
YOLOv8l-seg is the large model in the YOLOv8 family, designed for scenarios where accuracy takes precedence over speed. It delivers strong segmentation performance and is best suited for server-side deployments or high-performance edge hardware where greater computational resources are available. It is a strong choice for demanding applications such as medical imaging, industrial inspection, or autonomous systems.
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolov8l_seg
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/segmentation/yolov8l_seg/output/assets` folder

#### Model Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolov8l_seg dataset_root=<path to val2017>
```

- Output model and assets will be stored in the `modelzoo/segmentation/yolov8l_seg/output/compiled_model` folder
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../images model=yolov8l_seg
./flows/infer_hw.sh images_folder=../images model=yolov8l_seg
```

- Output images saved in `modelzoo/segmenation/yolov8l_seg/output/postprocessed_output_visualized`
...
```bash
cd ara-model-zoo
./flows/eval_float.sh dataset_root='../coco2017/' model=yolov8l_seg
./flows/eval_hw.sh dataset_root='../coco2017/' model=yolov8l_seg
```
