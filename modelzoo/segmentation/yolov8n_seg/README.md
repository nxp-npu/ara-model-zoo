...
YOLOv8n-seg is an instance segmentation model developed by ultralytics and a part of YOLO(You Only Look Once) family. The model is pretrained with Coco2017 dataset.

Yolov8n-seg is the smallest model in the YOLOv8 family. It's designed for environments where computational resources are limited, such as on mobile devices or edge devices. While it may not offer the same level of accuracy as the larger models, it's faster and requires less computational resources, making it a good choice for real-time object detection and segmenation tasks in resource-constrained environments.
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolov8n_seg
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/segmentation/yolov8n_seg/output/assets` folder

#### Model Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolov8n_seg dataset_root=<path to val2017>
```

- Output model and assets will be stored in the `modelzoo/segmentation/yolov8n_seg/output/compiled_model` folder
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../images model=yolov8n_seg
./flows/infer_hw.sh images_folder=../images model=yolov8n_seg
```

- Output images saved in `modelzoo/segmenation/yolov8n_seg/output/postprocessed_output_visualized`
...
```bash
cd ara-model-zoo
./flows/eval_float.sh dataset_root='../coco2017/' model=yolov8n_seg
./flows/eval_hw.sh dataset_root='../coco2017/' model=yolov8n_seg
```
