...
YOLOv8m-seg is an instance segmentation model developed by ultralytics and a part of YOLO(You Only Look Once) family. The model is pretrained with Coco2017 dataset.

YOLOv8m-seg is the medium model in the YOLOv8 family, positioned at the midpoint of the lineup. It offers a solid trade-off between accuracy and computational cost, making it a versatile choice for a wide range of real-world segmentation tasks. It is ideal for deployments on standard hardware or mid-range edge devices where higher accuracy is a priority but resources are still a consideration.
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolov8m_seg
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/segmentation/yolov8m_seg/output/assets` folder

#### Model Compile

```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolov8m_seg dataset_root=<path to val2017>
```

- Output model and assets will be stored in the `modelzoo/segmentation/yolov8m_seg/output/compiled_model` folder
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../images model=yolov8m_seg
./flows/infer_hw.sh images_folder=../images model=yolov8m_seg
```

- Output images saved in `modelzoo/segmenation/yolov8m_seg/output/postprocessed_output_visualized`
