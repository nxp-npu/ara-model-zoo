# Yolov8x Segmentation
## Use Case and High-Level Description

YOLOv8x-seg is an instance segmentation model developed by ultralytics and a part of YOLO(You Only Look Once) family. The model is pretrained with Coco2017 dataset.

YOLOv8x-seg is the largest and most powerful model in the YOLOv8 family, built for maximum segmentation accuracy. It is intended for use in high-resource environments such as data centers or workstations equipped with powerful GPUs, where inference speed is less of a constraint. It is the go-to option when achieving the highest possible accuracy is the primary objective, making it ideal for research, benchmarking, and production pipelines where precision is critical.
...
```bash
cd ara-model-zoo
./flows/precompiled_model_download.sh model=yolov8x_seg
```

- Precompiled models are compiled using Ara SDK.
- Output model and assets will be stored in the `modelzoo/segmentation/yolov8x_seg/output/assets` folder
...
```bash
cd ara-model-zoo
./flows/model_compile.sh model=yolov8x_seg dataset_root=<path to val2017>
```

- Output model and assets will be stored in the `modelzoo/segmentation/yolov8x_seg/output/compiled_model` folder
...
```bash
cd ara-model-zoo
./flows/infer_float.sh images_folder=../images model=yolov8x_seg
./flows/infer_hw.sh images_folder=../images model=yolov8x_seg
```

- Output images saved in `modelzoo/segmenation/yolov8x_seg/output/postprocessed_output_visualized`
...
```bash
cd ara-model-zoo
./flows/eval_float.sh dataset_root='../coco2017/' model=yolov8x_seg
./flows/eval_hw.sh dataset_root='../coco2017/' model=yolov8x_seg
```
